...
import {
  useCategoryList, useCreateCategoryMutation, useUpdateCategoryMutation, useDeleteCategoryMutation,
...
const EMPTY = { name: "", type: "handset", description: "" };

function CategoryDialog({ open, onOpenChange, record }) {
  const isEdit = Boolean(record);
  const { mutate: create, isPending: creating } = useCreateCategoryMutation();
  const { mutate: update, isPending: updating } = useUpdateCategoryMutation();
  const isPending = creating || updating;
  const [form, setForm] = useState(EMPTY);
  const [errors, setErrors] = useState({});

  useEffect(() => {
    setForm(record ? { name: record.name ?? "", type: record.type ?? "handset", description: record.description ?? "" } : EMPTY);
    setErrors({});
  }, [record, open]);

  const set = (e) => { setForm(f => ({ ...f, [e.target.name]: e.target.value })); setErrors(p => ({ ...p, [e.target.name]: undefined })); };
  const setSel = (name) => (value) => { setForm(f => ({ ...f, [name]: value })); setErrors(p => ({ ...p, [name]: undefined })); };
  ...
  const submit = (e) => {
    ...
    setErrors({});
    const opts = {
      onSuccess: () => { toast.success(isEdit ? "Category updated." : "Category added."); close(false); },
      onError: (err) => {
    ...
    <Dialog open={open} onOpenChange={close}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>{isEdit ? "Update category details." : "Add a product category."}</DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-3">
          <Field label="Category Name *" name="name" value={form.name} onChange={set} required placeholder="e.g. Flagship Phones" error={errors.name} />
          <div className="space-y-1.5">
            ...
            <Select value={form.type} onValueChange={setSel("type")}>
              <SelectTrigger className={cn(errors.type && "border-destructive focus-visible:ring-destructive/30")}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            {errors.type && <p className="text-xs text-destructive">{Array.isArray(errors.type) ? errors.type[0] : errors.type}</p>}
          </div>
          <DialogFooter className="pt-1">
            <Button type="submit" disabled={isPending}>{isPending ? "Saving…" : isEdit ? "Save Changes" : "Add Category"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
...
export function CategoriesTable() {
  ...
  const params = { page, page_size: PAGE_SIZE, ...(search && { search }), ...(typeFilter && { type: typeFilter }) };
  const { data, isLoading } = useCategoryList(params);
  ...
  return (
    <>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border/60 bg-muted/30">
                {["Category Name", "Type", "Description", ""].map((col, i) => (
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={4} className="px-4 py-14 text-center text-sm text-muted-foreground">No categories found.</td></tr>
              ) : rows.map(cat => (
                <tr key={cat.id} className="border-b border-border/40 last:border-0 hover:bg-muted/40 transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    <span className={cn("text-[11px] font-semibold px-2 py-0.5 rounded-full border capitalize",
                      TYPE_STYLES[cat.type] ?? "bg-muted text-muted-foreground border-border")}>
                      {cat.type}
                    </span>
                  </td>
                  <td className="px-4 py-3.5 text-muted-foreground max-w-64 truncate">{cat.description || "—"}</td>
                  <td className="px-4 py-3.5">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-32">
                        <DropdownMenuItem onClick={() => setEditRecord(cat)}>
                          <Pencil className="size-3.5" /> Edit
                        </DropdownMenuItem>
                        <DropdownMenuItem variant="destructive" onClick={() => setDeleteRecord(cat)}>
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-3.5 border-t border-border/60">
          {isLoading ? <Skeleton className="h-4 w-40 rounded-lg" /> : (
            <p className="text-xs text-muted-foreground">
              {totalCount === 0 ? "No categories" : `Showing ${start}–${Math.min(page * PAGE_SIZE, totalCount)} of ${totalCount} categories`}
            </p>
          )}
      ...
      <CategoryDialog open={dialogOpen} onOpenChange={setDialogOpen} record={null} />
      <CategoryDialog open={Boolean(editRecord)} onOpenChange={v => { if (!v) setEditRecord(null); }} record={editRecord} />
