...
import { useAccessoryStockList } from "@/hooks/useInventory";
...
function TableSkeleton() {
  return Array.from({ length: PAGE_SIZE }).map((_, i) => (
    <tr key={i} className="border-b border-border/40">
      <td className="px-4 py-3.5"><Skeleton className="h-4 w-36 rounded-lg" /></td>
...
export function AccessoryStockTable() {
  ...
  const { data, isLoading } = useAccessoryStockList(params);
  ...
  return (
    <div className="rounded-2xl border bg-card overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-border/60">
        <p className="text-sm font-medium flex-1">Accessory Stock</p>
        <select
          value={branchFilter}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border/60 bg-muted/30">
              {["Product", "Category", "Branch", "Qty In Hand", "Reorder Level", "Status"].map((col) => (
                <th
                  key={col}
              ...
              rows.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className={cn("px-4 py-3.5 font-bold tabular-nums", isLow && "text-yellow-600 dark:text-yellow-400")}>
                      {qty}
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      {isLow ? (
                        <span className="flex items-center gap-1.5 text-[11px] font-semibold text-yellow-600 dark:text-yellow-400">
