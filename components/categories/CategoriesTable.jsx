    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{isEdit ? "Update category details." : "Add a new category."}</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Field label="Name *" name="name" value={form.name} onChange={set} required placeholder="e.g. Phones" error={errors.name} />
...
function DeleteCategoryDialog({ open, onOpenChange, record }) {
...
export function CategoriesTable() {
  ...
  const { data, isLoading, refetch } = useCategoryList({ page, page_size: PAGE_SIZE, ...(search && { search }) });
  ...
  const handleExport = () => {
    ...
    exportToCSV(exportData, `categories_${new Date().toISOString().split("T")[0]}.csv`);
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-3.5 border-t border-border/60">
          {isLoading ? (
            ...
            <p className="text-xs text-muted-foreground">
              {totalCount === 0 ? "No categories" : `Showing ${start}–${end} of ${totalCount}`}
            </p>
          )}
      ...
      <CategoryDialog open={addOpen} onOpenChange={setAddOpen} record={null} />
      <CategoryDialog
